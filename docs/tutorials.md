...
Every tutorial is a single script in `docs/examples`, grouped by level (`level1/`, `level2/`,
`level3/`), written in jupytext's `py:percent` format.
...
* **run cell-by-cell**, as a notebook — use the 🚀 launch button at the top of any tutorial page to
  open it in Colab;
* **executed as a plain Python script**, top to bottom, with `python docs/examples/<level>/<name>.py`.
...
* unsure which flags to set → {doc}`examples/level1/configurations`
* ready to load your own CSV → {doc}`examples/level1/run_your_data`
