"""
Generate synthetic multi-task, multi-cluster datasets from GP priors (`generate_data`), and split a
dataset into the points a model observes and the points held out from it -- at random, to simulate
missingness (`RandomDataRemover`), or over a region of the input space (`SubdomainRemover`).
"""
...
def generate_grid(dims: Dimensions, config: ModelConfig, bounds: list[tuple[float, float]]) -> Grid:
	...
	blocks = [RegularGrid(bounds=(b,) * dims.I, n_points=grid_size).points for b in bounds]
...
class AbstractDataRemover(eqx.Module):
	"""
	Base class for modules that split a Dataset into the points they keep and the points they remove.

	A subclass implements `removal_mask` alone -- which points go -- and takes whatever configures
	that choice (a PRNG key, bounds, ...) at construction. Applying the mask to both halves is this
	class's job.
	"""

	@abstractmethod
	def removal_mask(self, dataset: Dataset, *args, **kwargs) -> Array:
		"""
		Boolean `(T, O*N, C)` mask of the points to remove from `dataset`.
		"""
		...

	def __call__(self, dataset: Dataset, *args, **kwargs) -> tuple[Dataset, Dataset]:
		"""
		Split `dataset` in two, on `removal_mask`.
		...
		dataset
			Dataset to split.
		*args, **kwargs
			Forwarded to the subclass's `removal_mask`.

		Returns
		-------
		kept, removed
			`dataset` without the removed points, and `dataset` reduced to them. A point already
			missing in `dataset` is missing in both.

		Notes
		-----
		A point a half excludes is marked by NaN in `outputs` (and in `known_output_noise`); this
		masking is read downstream in `mimosa.hyperpost`, `mimosa.nll` and `mimosa.prediction`.
		`inputs` is never touched, so a single `Grid` and `Dimensions` describe both halves.
		"""
		mask = self.removal_mask(dataset, *args, **kwargs)
		return self._mask_out(dataset, mask), self._mask_out(dataset, ~mask)

	@staticmethod
	def _mask_out(dataset: Dataset, mask: Array) -> Dataset:
		"""
		`dataset` with every point `mask` selects turned into a missing value.
		"""
		return Dataset(
			inputs=dataset.inputs,
			outputs=jnp.where(mask, jnp.nan, dataset.outputs),
			known_output_noise=None
			if dataset.known_output_noise is None
			else jnp.where(mask, jnp.nan, dataset.known_output_noise),
			output_ids=dataset.output_ids,
		)

	@staticmethod
	def _layout(dataset: Dataset) -> tuple[int, int, int, int]:
		"""
		`(T, N, O, C)` of `dataset`: tasks, points per task and per output, outputs, and channels.
...
class RandomDataRemover(AbstractDataRemover):
	"""
	Remove data points at random, per `DataRemovalConfig`.

	Attributes
	----------
	key
		`jax.random` PRNG key the missingness is drawn from.
	"""

	key: Array

	def removal_mask(self, dataset: Dataset, config: DataRemovalConfig) -> Array:
		"""
		See `AbstractDataRemover.removal_mask`.
		...
		dataset
			Dataset to split.
		config
			How many points go missing per task, and which axes share that missingness.

		Raises
		------
		ValueError
			If `config.same_missing_across_outputs` but the outputs do not share input locations.
		"""
		T, N, O, C = self._layout(dataset)
		...
		# "Which k of N points are removed" is drawn without replacement by ranking iid uniform scores
		# per row and keeping the k lowest ranks: no python loop, no vmap, works whether the row is
		# (task,), (task, output) or (task, output, channel). Rows are drawn once and broadcast over
		# the axes missingness is shared along.
		o = 1 if config.same_missing_across_outputs else O
		shape = (T, o, N) if config.same_missing_across_channels else (T, o, C, N)
		key_scores, key_counts = jr.split(self.key)
		...
		return jnp.broadcast_to(selected, (T, O, N, C)).reshape(T, O * N, C)


class SubdomainRemover(AbstractDataRemover):
	"""
	Split a Dataset over an axis-aligned region of the input space, optionally restricted to some
	tasks, cluster, channels or outputs.

	A point is *in the subdomain* when every one of its input dimensions lies within `bounds`. Use it
	to hold data out on purpose: a stretch of the domain to interpolate over, or a tail to forecast.

	Attributes
	----------
	bounds
		One `(min, max)` per input dimension, both inclusive, as in `mimosa.grid.RegularGrid`.

	Examples
	--------
	>>> observed, held_out = SubdomainRemover(bounds=((0.0, 1.5),))(dataset)  # doctest: +SKIP
	>>> observed, held_out = SubdomainRemover(bounds=((0.0, 1.5),))(dataset, t_id=3)  # doctest: +SKIP
	"""
	...
	def removal_mask(
		...
		dataset
			Dataset to split.
		t_id, k_id, c_id, o_id
			`"all"` (default) or an int: restrict the removal to a single task, cluster, channel or
			output. They intersect -- `t_id=3, c_id=0` removes the subdomain of task 3's channel 0
			alone. A task outside the restriction keeps all of its points.
		mixture
			Clustering `k_id` indexes, through its hard `assignments`. Required by `k_id`, ignored
			otherwise.

		Raises
		------
		ValueError
			If `bounds` does not hold one pair per input dimension, or if `k_id` is given without a
			`mixture`.
		"""
		T, N, O, C = self._layout(dataset)
		I = dataset.inputs.shape[-1]
		if len(self.bounds) != I:
			raise ValueError(f"Expected one (min, max) per input dimension, got {len(self.bounds)} for I={I}.")
		if k_id != "all" and mixture is None:
			raise ValueError("Restricting to a cluster needs the `mixture` its assignments come from.")

		low, high = jnp.asarray(self.bounds).T
		inside = jnp.all((dataset.inputs >= low) & (dataset.inputs <= high), axis=-1)  # (#T, oN), NaN padding is False

		# `inputs` holds a single block of N points when the outputs share their locations, against
		# `outputs`' O blocks; and a single task when they are isotopic, against `outputs`' T.
		if inside.shape[1] != dataset.outputs.shape[1]:
			inside = jnp.tile(inside, (1, O))
		inside = jnp.broadcast_to(inside, dataset.outputs.shape[:2])

		tasks = jnp.zeros(T, bool).at[jnp.asarray(_resolve_ids(t_id, T))].set(True)
		if k_id != "all":
			tasks &= mixture.assignments == _resolve_ids(k_id, mixture.responsibilities.shape[1])[0]

		channels = jnp.zeros(C, bool).at[jnp.asarray(_resolve_ids(c_id, C))].set(True)

		if o_id == "all":
			rows = jnp.asarray(True)
		elif dataset.output_ids is None:
			o = _resolve_ids(o_id, O)[0]
			rows = jnp.zeros(O * N, bool).at[o * N : (o + 1) * N].set(True)  # output-major, see `sample_inputs`
		else:
			# `(#T, O*N)`: the outputs' rows may differ per task, which broadcasts against `inside`.
			rows = dataset.output_ids == _resolve_ids(o_id, O)[0]

		return (inside & tasks[:, None] & rows)[..., None] & channels
